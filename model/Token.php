...
class Token extends Database
{
    public function rowCountToken($token, $mail): int
    {
        ...
        return $this->db()->query("select * from TOKEN 
            inner join USER on USER.id = TOKEN.id_user 
    ...
    {
        $token = $this->db()->quote($token);
        $this->db()->query("delete from TOKEN where token like $token");
    }
    ...
    public function setToken($token, $id_user)
    {
        ...
        $this->db()->query("insert into TOKEN set token=$token, id_user=$id_user, expire=date_add(curdate(), interval 1 year)");
    }
    ...
    public function getUserWithToken($token, $mail)
    {
        $token = $this->db()->quote($token);
        $mail = $this->db()->quote($mail);
        return $this->db()->query("select u.id, pseudo, mail, droits, nombre, icone, life, last_login, theme, kanji, background from TOKEN t
